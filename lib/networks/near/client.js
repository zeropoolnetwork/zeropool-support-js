...
class NearClient extends client_1.Client {
    static async create(config, poolAddress) {
        ...
        self.near = await (0, near_api_js_2.connect)(config);
        self.wallet = new near_api_js_2.WalletConnection(self.near, 'zeropool');
        self.config = config;
        self.poolContract = new near_api_js_2.Contract(self.wallet.account(), poolAddress, {
    ...
    async approve(_tokenAddress, _spender, amount) {
        ...
        await this.poolContract.lock({
            amount: amount
        }, near_api_js_1.DEFAULT_FUNCTION_CALL_GAS, amount);
    }
    async getAddress() {
